...
void DeviceVolume::refresh()
{
    ...
    if (deviceID != 0)
        for (UInt32 el : { (UInt32) kAudioObjectPropertyElementMain, (UInt32) 1 })
        {
            ...
            if (AudioObjectHasProperty ((AudioObjectID) deviceID, &a)) { dbElement = (int) el; break; }
        }
}

float DeviceVolume::getVolumeDb() const
{
    if (deviceID == 0 || dbElement < 0) return 0.0f;
...
{
    if (deviceID == 0) return 0.0f;
    const auto addr = volumeAddr (caScope (scope));
    if (! AudioObjectHasProperty ((AudioObjectID) deviceID, &addr)) return 0.0f;
    Float32 v = 0.0f;
    UInt32 sz = sizeof (v);
    if (AudioObjectGetPropertyData ((AudioObjectID) deviceID, &addr, 0, nullptr, &sz, &v) != noErr)
        return 0.0f;
...
bool DeviceVolume::getMute() const
{
    ...
    if (! AudioObjectHasProperty ((AudioObjectID) deviceID, &addr)) return false;
    UInt32 m = 0;
    UInt32 sz = sizeof (m);
