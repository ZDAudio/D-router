                                   ...
                                   unsigned int complexity)
{
    ...
    if (s != noErr || converter == nullptr) return false;
...
{
    if (bypass || converter == nullptr) return 0;
...
int SampleRateConverter::pullOutput (float* dst, int numSamples)
{
    ...
    OSStatus s = AudioConverterFillComplexBuffer (converter, &inputProc, this,
                                                  &outPackets, &abl, nullptr);
    if (s != noErr && s != 1) return 0;
