...
public:
    ...
    void mouseDrag (const juce::MouseEvent& e) override
    {
        ...
        const float f = freqForX (juce::jlimit ((float) curveArea.getX(),
                                                 (float) curveArea.getRight(), e.position.x));
        ...
        if (type <= 2)
        {
            const float gainDb = gainForY (juce::jlimit ((float) curveArea.getY(),
                                                         (float) curveArea.getBottom(), e.position.y));
    ...
    void mouseWheelMove (const juce::MouseEvent& e, const juce::MouseWheelDetails& w) override
    {
        ...
        if (b < 0) { viewport.mouseWheelMove (e, w); return; }
...
private:
    ...
    {
        if (auto* a = eq.getValueTreeState().getRawParameterValue (id)) return a->load();
        return 0.0f;
    }
    ...
    {
        return juce::jmap (juce::jlimit (-kGainSpan, kGainSpan, db), kGainSpan, -kGainSpan,
                           (float) curveArea.getY(), (float) curveArea.getBottom());
    }
    float gainForY (float y) const
    {
        return juce::jmap (y, (float) curveArea.getY(), (float) curveArea.getBottom(),
                           kGainSpan, -kGainSpan);
    }
    ...
    juce::dsp::IIR::Coefficients<float>::Ptr bandCoeffs (int b) const
    {
        ...
        switch (type)
        {
            case 1: return juce::dsp::IIR::Coefficients<float>::makeLowShelf  (sr, freq, q, gainLin);
            case 2: return juce::dsp::IIR::Coefficients<float>::makeHighShelf (sr, freq, q, gainLin);
            ...
            default: return juce::dsp::IIR::Coefficients<float>::makePeakFilter (sr, freq, q, gainLin);
        }
    }
    ...
    int nodeHitTest (juce::Point<float> p) const
    {
        for (int b = 0; b < ParametricEQ::kNumBands; ++b)
        {
            ...
            if (pos.getDistanceFrom (p) < 12.0f) return b;
        }
        return -1;
    }
    ...
    void drawCurve (juce::Graphics& g)
    {
        ...
        for (float db = -kGainSpan; db <= kGainSpan + 0.1f; db += 12.0f)
        {
            ...
            g.drawText (juce::String ((int) db) + " dB",
                        curveArea.getX() + 2, (int) y - 12, 44, 12,
        ...
        for (int b = 0; b < ParametricEQ::kNumBands; ++b)
        {
            ...
            g.drawText (juce::String (b + 1), (int) pos.x - 5, (int) pos.y - 6, 10, 12,
