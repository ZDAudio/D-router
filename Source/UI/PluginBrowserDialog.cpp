...
namespace
{
    juce::File getPluginListCacheFile()
    {
        return juce::File::getSpecialLocation (juce::File::userApplicationDataDirectory)
                    .getChildFile ("dcorerouter").getChildFile ("au-list.xml");
    }
}
...
PluginBrowserDialog::PluginBrowserDialog (AudioEngine& e) : engine (e)
{
    ...
    if (filteredList.empty()) startScan();
    else status.setText (juce::String (filteredList.size()) + " plugins loaded from cache.",
...
void PluginBrowserDialog::startScan()
{
    ...
    scanner = std::make_unique<juce::PluginDirectoryScanner> (
        engine.getKnownPluginList(), fmt, paths, true,
        juce::File{}, false);
...
{
    if (row < 0 || row >= (int) filteredList.size()) return;
...
void PluginBrowserDialog::ListModel::paintListBoxItem (int row, juce::Graphics& g,
                                                       int w, int h, bool selected)
{
    if (row < 0 || row >= (int) dialog.filteredList.size()) return;
