...
{
    if (currentLevel < 1.0e-5f) return 0;
...
void LevelMeter::paint (juce::Graphics& g)
{
    ...
    auto colourForIndex = [] (int i) -> juce::Colour
    {
        ...
        if (i >= numSegments - 1) return juce::Colour::fromRGB (230, 40, 40);
        if (i >= numSegments - 2) return juce::Colour::fromRGB (220, 140, 20);
    ...
    else
    {
        ...
        {
            g.setColour (colourForIndex (i));
            g.fillRect (r.getX(), r.getBottom() - segH - (float) i * (segH + gap),
                        r.getWidth(), segH);
        }
    }
}
